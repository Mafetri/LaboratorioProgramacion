...
turns.getFutureTurns = async (approved) => {
    try {
        // If approved is undefined, it searchs the turns that hasn't been revised yet
        if(approved == "unchecked") {
            const [rows] = await pool.query("SELECT turns.*, u.name AS requester_name, u.surname AS requester_surname, i.name AS instructor_name, i.surname AS instructor_surname FROM turns LEFT JOIN users u ON turns.user_dni = u.dni LEFT JOIN users i ON turns.instructor_dni = i.dni WHERE turns.end_date > CURDATE() AND turns.approved IS NULL ORDER BY turns.request_date DESC");
            return rows;
        } else if (approved == "true"){
            // If approved is true, then it returns the approved turns, else, it returns the false ones
            const [rows] = await pool.query("SELECT turns.*, u.name AS requester_name, u.surname AS requester_surname, i.name AS instructor_name, i.surname AS instructor_surname FROM turns LEFT JOIN users u ON turns.user_dni = u.dni LEFT JOIN users i ON turns.instructor_dni = i.dni WHERE turns.end_date > CURDATE() AND turns.approved = 1 ORDER BY turns.start_date ASC");
            return rows;
        }

        // If approved is undefined, then it returns the approved turns and the unchecked turns
        const [rows] = await pool.query("SELECT turns.*, u.name AS requester_name, u.surname AS requester_surname, i.name AS instructor_name, i.surname AS instructor_surname FROM turns LEFT JOIN users u ON turns.user_dni = u.dni LEFT JOIN users i ON turns.instructor_dni = i.dni WHERE turns.end_date > CURDATE() AND (turns.approved = 1 OR turns.approved IS NULL) ORDER BY turns.start_date ASC");
        return rows;
	} catch (error) {
		throw error;
	}
...
turns.getFutureTurnsUser = async (dni) => {
    try {
        const [rows] = await pool.query("SELECT turns.*, users.name, users.surname FROM turns LEFT JOIN users ON turns.instructor_dni = users.dni WHERE end_date > CURDATE() AND turns.user_dni = ? AND purpose <> 'workshop' AND purpose <> 'baptism' ORDER BY turns.start_date ASC", [dni]);
        return rows;
    } catch (error) {
        throw error;
    }
...
turns.deletePastTurns = async () => {
    try {
        const [dbRes] = await pool.query("DELETE FROM turns WHERE end_date < DATE_SUB(CURDATE(), INTERVAL ? DAYS)", [DAYS_UNTIL_CLEAN]);
        return dbRes;
    } catch (error) {
        throw error;
    }
